...
def print_maze( maze: Maze, path = None, cost = None, verbose = False ) :
    print()
...
def fall( coordinates, maze, nb_turns, verbose = False ) :
    ...
    for coord in fallen :
        ...
        maze.tiles.pop( coord )
    return remaining, maze

def do_problem( str_data: str, maze_max: int, nb_turns: int, verbose = False ) :
    coordinates, maze = parse_data( str_data, maze_max, verbose )
    print_maze( maze )
    coordinates, maze = fall( coordinates, maze, nb_turns, verbose )
    print_maze( maze )
    path, cost = a_star( maze, heuristic_distance )
    print_maze( maze, path, cost )
    print( f"END" )

def do_tests( i = None ) :
    str_data = get_file_content( get_test_file_path( i ) )
    do_problem( str_data, 6, 12, True )

def do_input() :
    str_data = get_file_content( get_input_file_path() )
    do_problem( str_data, 70, 1024, False )
