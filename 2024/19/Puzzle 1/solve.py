...
def do_problem( str_data: str, verbose = False ) :
    ...
    possible = [ d for d in designs if design_possible( d, towel_patterns, verbose ) ]
    if verbose :
        print( f"Possible designs: {', '.join(possible)}" )
    print( f"Nb of possible designs found: {len(possible)}" )
    print( f"END" )
